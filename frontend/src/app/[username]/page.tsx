...
import styles from './page.module.css';
   ...
   async function fetchProfile(username: string): Promise<{ data: Profile | null, error: string | null, message: string | null }> {
     try {
       ...
       if (!res.ok) {
         return { data: null, error: 'FETCH_ERROR', message: `Server responded with ${res.status}: ${res.statusText}` };
       }
       ...
       return {
         data: null,
         error: 'NETWORK_ERROR',
         message: e instanceof Error ? e.message : 'A network error occurred while fetching the profile.'
       };
     }
   }
        ...
        <section className={styles.section}>
          ...
          <div className={styles.statsGrid}>
            <div className={styles.statBlock}>
              ...
              <p className={styles.statValue}>{profile.stats.total_contributions}</p>
            </div>
            <div className={styles.statBlock}>
            ...
            <div className={styles.statBlock}>
              ...
              <p className={styles.statValue}>{profile.stats.pinned_repos.length}</p>
            </div>
          </div>

          <div className={styles.repoGrid}>
            {profile.stats.pinned_repos.map((repo) => (
              <a key={repo.name} href={repo.url} target="_blank" className={styles.repoCard}>
